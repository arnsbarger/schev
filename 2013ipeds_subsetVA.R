s# subset IPEDS data for VA only
library(dplyr)

setwd("~/Google Drive/SCHEV (Peter Blake - Wendy Kang)/Data/IPEDS/")

# load hd2013 ipeds table - only relevant variables (see hd2013 dictionary) ####
hd2013 <- read.csv("Institutional Characteristics/hd2013.csv")   

# keep only VA schools
hd2013 <- hd2013 %>% filter(STABBR == "VA")

# add ic2013_ay
ic2013_ay <- read.csv("Institutional Characteristics/ic2013_ay.csv")
hd_icay2013 <- merge(hd2013, ic2013_ay, by = "UNITID", all.x = TRUE, all.y = FALSE)

# add financial aid data
fanp <- read.csv("Student Financial Aid and Net Price/SFA1314/sfa1314.csv", na.strings=c("",".","NA"))
hd_icay_fanp2013 <- merge(hd_icay2013, fanp, by = "UNITID", all.x = TRUE)

# add admissions data
admissions <- read.csv("Admissions and Test Scores/IC2013/ic2013.csv")
ipeds_final <- merge(hd_icay_fanp2013, admissions, by = "UNITID", all.x = TRUE)
...
colnames(ipeds_final) <- paste(colnames(ipeds_final), "1314", sep = "")

ipeds_final[ipeds_final=='.'] <- NA

write.csv(ipeds_final, "output/ipeds_subset1314.csv", row.names = FALSE)
